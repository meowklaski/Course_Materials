...
class KNearestNeighbor(object):
    ...
    def compute_distances_two_loops(self, X):
        ...
        in self.data using a nested loop over both the training data and the
        test data.
        ...
        for i in xrange(num_test):
            for j in xrange(num_train):
                dists[i,j] = np.sum((self.data[j] - X[i])**2)
        return np.sqrt(dists)

    def compute_distances_one_loop(self, X):
        """
        Compute the distance between each test point in X and each training point
        in self.data using a single loop over the test data.
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in xrange(num_test):
            ...
            # testing point, and use self.labels to find the labels of these       #
